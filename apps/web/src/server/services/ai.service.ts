...
function buildSystemPrompt(userId: string): string {
  ...
  const catData = categories.map((c) => ({
    ...
    subCategories: subCategories.filter((s) => s.categoryId === c.id).map((s) => ({ id: s.id, name: s.name })),
  }));

  const habitData = habits.map((h) => ({
    id: h.id, name: h.name, type: h.type, xpReward: h.xpReward, streak: h.streak, categoryId: h.categoryId, subCategoryId: h.subCategoryId,
  }));

  const goalData = goals.map((g) => ({
    id: g.id, title: g.title, type: g.type, targetValue: g.targetValue, currentValue: g.currentValue, xpReward: g.xpReward,
  }));
    ...
    try {
      switch (action.type) {
        case 'create_category': {
          ...
          const subList = action.subCategories?.length ? ` with subcategories: ${action.subCategories.join(', ')}` : '';
        ...
        case 'create_habit': {
          ...
          if (action.categoryName) {
            ...
            const match = cats.find((c) => c.name.toLowerCase() === action.categoryName!.toLowerCase());
            if (match) {
              categoryId = match.id;
              if (action.subCategoryName) {
                const subs = freshDb.subCategories.filter((s) => s.categoryId === match.id);
                const subMatch = subs.find((s) => s.name.toLowerCase() === action.subCategoryName!.toLowerCase());
                if (subMatch) subCategoryId = subMatch.id;
              }
            }
          }

          const habitType = (['YES_NO', 'HOURS', 'MANUAL'].includes(action.habitType ?? '') ? action.habitType : 'YES_NO') as HabitType;
        ...
        case 'complete_habit': {
          ...
          if (!matched) {
            ...
            matched = userHabits.find((h) => words.some((w) => w.length > 2 && h.name.toLowerCase().includes(w)));
          }

          if (matched) {
            const result = await habitService.completeHabit(matched.id, userId, undefined, action.hoursLogged);
        ...
        case 'create_goal': {
          ...
          if (action.categoryName) {
            ...
            const cat = freshDb3.categories.find((c) => c.userId === userId && c.name.toLowerCase() === action.categoryName!.toLowerCase());
            if (cat) categoryId2 = cat.id;
          }

          const goalType = (['SHORT_TERM', 'LONG_TERM'].includes(action.goalType ?? '') ? action.goalType : 'SHORT_TERM') as GoalType;
        ...
        case 'log_xp': {
          ...
          if (action.categoryName) {
            ...
            const cat = freshDb4.categories.find((c) => c.userId === userId && c.name.toLowerCase() === action.categoryName!.toLowerCase());
