...
const DEFAULT_LEVEL_RANK_MAP: Record<string, string> = {
  '10': 'D', '20': 'C', '30': 'B', '45': 'A', '60': 'S', '80': 'SS', '100': 'SSS',
};

const DEFAULT_RANK_TITLES: Record<string, string> = {
  E: 'Novice', D: 'Aspiring', C: 'Rising Star', B: 'Adventurer', A: 'Veteran', S: 'Expert', SS: 'Master', SSS: 'Legend',
};

const DEFAULT_ARTIFACT_THRESHOLDS: Record<string, string> = {
  '500': 'Common', '2000': 'Rare', '5000': 'Epic', '15000': 'Legendary', '50000': 'Mythic',
};

const DEFAULT_STAT_MULTIPLIERS: Record<string, number> = {
  streak: 1.5, bonus: 2.0, manual: 1.0, auto: 1.0,
};
...
export async function updateRulebook(
  ...
  if (data.artifactThresholds !== undefined) config.artifactThresholds = JSON.stringify(data.artifactThresholds);
  if (data.statMultipliers !== undefined) config.statMultipliers = JSON.stringify(data.statMultipliers);
