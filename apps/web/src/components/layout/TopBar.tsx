...
export function TopBar() {
  ...
  return (
    <header
      className={clsx(
        'fixed top-0 right-0 z-50 border-b theme-transition fixed-gpu',
      ...
      style={{
        ...
        /* Safe-area spacer — pushes content below the notch/dynamic island */
        paddingTop: 'env(safe-area-inset-top, 0px)',
      }}
    >
      {/* Inner content bar — fixed height, never squashed by safe-area */}
      <div className="flex items-center justify-between h-12 px-3 md:h-14 md:px-4 md:pr-6">
        {/* Logo / Title */}
        <Link href="/" className="flex items-center gap-1.5 md:gap-2 group shrink-0">
          <div
            className="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center rounded-md border transition-all group-hover:scale-105"
            style={{
          ...
          <h1 className="font-heading text-[11px] md:text-base tracking-wider animate-text-shimmer">
            LIFE QUEST
          </h1>
        </Link>
